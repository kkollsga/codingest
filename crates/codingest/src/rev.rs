...
use kglite::api::mutation::extend_graph;
...
fn stamp_node_revs(graph: &mut DirGraph, node_revs: &NodeRevManifest) -> Result<(), String> {
    let revs_key = graph
        .interner
        .try_get_or_intern("revs")
        .expect("intern revs");
    let fp_key = graph
        .interner
        .try_get_or_intern("rev_fp")
        .expect("intern rev_fp");
...
fn stamp_edge_revs(graph: &mut DirGraph, edge_revs: &EdgeRevManifest) {
    ...
    // `graph.graph.edge_weight_mut` is a `GraphWrite` trait method (that trait is
    // not exported by `kglite::api`); `DirGraph::_get_connection_mut` is its
    // public inherent wrapper, calling through to the identical backend method.
    for (eidx, revs_val) in updates {
        if let Some(edge) = graph._get_connection_mut(eidx) {
...
mod tests {
    ...
    fn merged_graph_revs_survive_save_reload() {
        ...
        let reloaded = kglite::api::io::load_file(out.to_str().unwrap()).expect("reload");
        assert_eq!(
